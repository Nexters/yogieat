...
import { ResultGeneratingSpeechBubble } from "#/pageComponents/gathering/opinion/result/ResultGeneratingSpeechBubble";

const DRUMROLL_DURATION_MS = 3000;

interface RandomPickDrumrollPageProps {
	accessKey: string;
	restaurantId: number;
}

export const RandomPickDrumrollPage = ({
	accessKey,
	restaurantId,
}: RandomPickDrumrollPageProps) => {
	const router = useRouter();

	useEffect(() => {
		const timer = setTimeout(() => {
			router.push(
				`/gathering/${accessKey}/opinion/result/random-pick/${restaurantId}`,
			);
		}, DRUMROLL_DURATION_MS);

		return () => clearTimeout(timer);
	}, [router, accessKey, restaurantId]);
